...
* @li <b>i</b>：大小写不敏感的修饰符。
*
* @li <b>m</b>：多行匹配，默认情况下，PCRE认为目标字符串是一组单行字符组成的
* (然而实际上它可能会包含多行)。元字符“^”仅匹配字符串的开始位置，而元字符“$”则仅
* 匹配字符串的末尾，或者新行之前(除非设置了D修饰符)。使用后，“^”和“$”会匹配每一行
* 的开始和结尾。如果目标字符串中没有“/n”字符或者模式中没有出现“^”和“$”，
* 则设置此修饰符是没有任何影响的。
*
* @li <b>s</b>：用来增强字符类“.”(点号)的功能，默认情况下，点号匹配任意字符，
* 但不包括换行符。使用此修饰符后，可以匹配任意字符包括换行符。
*
* @li <b>x</b>：如果设置了这个修饰符，模式中的空白字符，除非被转义或者在一个字符类中, 
* 否则都被忽略。在一个未转义的“#”之后直到下一个换行符之间的内容都会被正则表达式引擎
* 忽略以方便添加注释。
*
* @li <b>A</b>：如果设置了此修饰符，模式被强制成为“anchored”(锚点)，
* 也就是说约束使其始终从字符串的最前面开始搜索。 这个影响也可以通过构造适当的规则来实现。
*
* @li <b>E</b>：如果设置了这个修饰符，“$”会匹配目标字符串的结尾之前，而如果没有设置
* 这个修饰符，如果目标字符串最后一个字符是换行符，“$”就会匹配该换行符之前而不是字符串结尾之前。
*
* @li <b>G</b>：这个修饰符逆转了贪婪的模式，如果没有这个设置，默认情况下的正则匹配时贪婪的，
* 量词后增加了“?”的模式是非贪婪的，但是如果设置了这个修饰符，则刚好相反。
*
* @li <b>R</b>：约束pcre选项相对于前一个匹配项成功匹配的串尾开始搜索。
*
* @li <b>U</b>：约束pcre选项匹配解码后的URI缓冲区（类似于uricontent和http_uri)。
*
...
* 例："\b(\w+)\b\s+\1\b"可以用来匹配重复的单词，像"go go", 或者"kitty kitty"。 这个表达式首先是一个单词，
...
* 匹配以"ing"结尾的单词的前面部分(除了"ing"以外的部分)，如查找"I'm singing while you're dancing"。时，它会匹配"sing"和"danc"。@n
* “(?<=exp)”也叫零宽度正回顾后发断言：它断言自身出现的位置的前面能匹配表达式exp。比如"(?<=\bre)\w+\b"会匹配以"re"开头的单词的后半部分
* (除了"re"以外的部分)，例如在查找"reading a book"时，它匹配"ading"。
* 
* @li 负向零宽断言@n
...
* 例："a.*b"，它将会匹配最长的以a开始，以b结束的字符串。如果用它来搜索"aabab"的话，它会匹配整个字符串"aabab"。
* 
* @li 处理选项@n
...
* 例："a.*?b"匹配最短的，以"a"开始，以"b"结束的字符串。如果把它应用于"aabab"的话，它会匹配"aab"（第一到第三个字符）和"ab"（第四到第五个字符）。@n
* @li	懒惰限定符@n
* "*?":		重复任意次，但尽可能少重复。@n
* "+?":		重复1次或更多次，但尽可能少重复。@n
* "??":		重复0次或1次，但尽可能少重复@n
* "{n,m}?":	重复n到m次，但尽可能少重复。@n
* "{n,}?":	重复n次以上，但尽可能少重复。@n
*
