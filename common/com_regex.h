...
*			使用后，“^”和“$”会匹配每一行的开始和结尾。如果目标字符串中没有“/n”字符或者模式中没有出现“^”和“$”，则设置此修饰符是没有任何影响的。
...
*			这个影响也可以通过构造适当的规则来实现。
* @li	<b>E</b>：如果设置了这个修饰符，“$”会匹配目标字符串的结尾之前，而如果没有设置这个修饰符，如果目标字符串最后一个字符是换行符，
*			$就会匹配该换行符之前而不是字符串结尾之前。	
* @li	<b>G</b>：这个修饰符逆转了贪婪的模式，如果没有这个设置，默认情况下的正则匹配时贪婪的，量词后增加了“?”的模式是非贪婪的，但是如果设置了这个修饰符，则刚好相反。 
