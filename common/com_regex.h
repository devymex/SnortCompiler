...
*
* @li	<b>i</b>：大小写不敏感的修饰符
* @li	<b>m</b>：多行匹配, 默认情况下，PCRE认为目标字符串是一组单行字符组成的(然而实际上它可能会包含多行)。 
*			元字符^仅匹配字符串的开始位置，而元字符$则仅匹配字符串的末尾，或者新行之前(除非设置了D修饰符)。
*			使用后，^和$会匹配每一行的开始和结尾。如果目标字符串中没有/n字符或者模式中没有出现^和$，则设置此修饰符是没有任何影响的。
* @li	<b>s</b>：用来增强字符类.(点号)的功能，默认情况下，点号匹配任意字符，但不包括换行符。使用此修饰符后，可以匹配任意字符包括换行符。
* @li	<b>x</b>：如果设置了这个修饰符，模式中的空白字符，除非被转义或者在一个字符类中，否则都被忽略。 
*			在一个未转义的#之后直到下一个换行符之间的内容都会被正则表达式引擎忽略以方便添加注释。
* @li	<b>e</b>：使用此修饰符后，preg_replace的$replacement参数除了可以使用正常的反向引用来取得匹配值，
			还可以在其中书写eval语法允许的字符串进行求值，并将返回结果用于最终的替换。
* @li	<b>A</b>：如果设置了此修饰符，模式被强制成为”anchored”(锚点)，也就是说约束使其始终从字符串的最前面开始搜索。 
*			这个影响也可以通过构造适当的规则来实现。
* @li	<b>D</b>：如果设置了这个修饰符， $会匹配目标字符串的结尾之前，而如果没有设置这个修饰符，如果目标字符串最后一个字符是换行符，
*			$就会匹配该换行符之前而不是字符串结尾之前。如果设置了m修饰符，则这个修饰符会被忽略。
* @li	<b>S</b>：当一个模式需要多次使用的时候，我们为了获得更高的匹配效率，值得花费一些时间对其进行分析。如果设置了这个修饰符， 
*			将会进行这个额外的分析。当前，这种对一个模式的分析仅仅适用于非锚点的匹配(即没有一个单一固定的开始字符)。
* @li	<b>U</b>：这个修饰符逆转了贪婪的模式，如果没有这个设置，默认情况下的正则匹配时贪婪的，量词后增加了?的模式是非贪婪的，但是如果设置了这个修饰符，则刚好相反。
* @li	<b>X</b>：这个修饰符启用了一个PCRE中与perl不兼容的额外功能。任意反斜线后面跟一个没有特殊含义的字符会导致一个错误，以此来保留这些组合以备后期扩展。
*			默认情况下，和perl一样，反斜线后跟一个没有特殊含义的字符会以该字符原意解释。
* @li	<b>J</b>：允许子组有重复的名字。
* @li	<b>u</b>：模式字符串被认为是UTF-8的。 
*
