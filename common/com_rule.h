...
* 
* 以下为可以处理的Snort规则选项列表
* @li content：Snort规则模式选项。
* @li nocase：content及uricontent选项的修饰选项。
* @li offset：content及uricontent选项的修饰选项。
* @li depth：content及uricontent选项的修饰选项。
* @li distance：content及uricontent选项的修饰选项。
* @li within：content及uricontent选项的修饰选项。
* @li uricontent：Snort规则模式选项。
* @li pcre：Snort规则模式选项。
*
...
* 
* 模式选项有两种匹配位置：@n
* (1)从数据包净载中的开始位置进行匹配，称为
* @b起点匹配；@n
* (2)以前一个选项成功匹配结束后净载的位置为起点，经过指定数值范围的跳转后再开始匹配，称为
* @b连续匹配。@n
*
...
* 将规则中需要从数据包净载的起始位置开始匹配的一个选项，以及该选项之后的多个连续匹配选项按顺序连接在一起，称为
* @b选项链。
...
*
* @section subRuleSignature 规则的特征字符串
* 分组后每一组要选出一个或多个全局唯一的特征字符串（Signature）来代表该组，因此从选项链中提取Signature是分组的前提。
* @subsection subConSig 从content和uricontent中提取Signature
*
...
*
* @subsection subPcreSig 从pcre中提取Signature
* 
*/
