...
*
* @ref CDfa中的每一个状态都有一个标记，如果是终态，则标记为FINAL，否则标记为NORMAL。
* 另外，@ref CDfa 中还有一个标记用来指定起始状态。
*
* @section secDeterm NFA的确定化
* 
* 从NFA到DFA的转换采用经典的子集构造算法。@n
* 子集构造算法的基本思想为让构造得到的DFA的没一个个状态对应于NFA的一个状态集合。DFA在读入字符a1,a2,...an之后
* 到达的状态对应于相应NFA从初始状态出发，沿着以a1,a2,...an为标号的路径能到达的状态集合。
*
