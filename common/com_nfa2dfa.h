...
*在匹配过程中，若数据包匹配到了某一终态，则该数据包能匹配到该终态对应的所有的DFA。
* 
...
* 子集构造算法的基本思想为让构造得到的DFA的每一个状态对应于NFA的一个状态集合。
*DFA在读入字符a1,a2,...an之后到达的状态对应于相应NFA从初始状态出发，
*沿着以a1,a2,...an为标号的路径能到达的状态集合。
*
* @section secMinimize DFA的最小化
* 
* 采用Hopcroft于1971年提出的最小化算法。@n
* 最小化算法的基本思想为将一个DFA的状态集合划分成多个组，每个组中的各个状态之间相互不可区分。
*当任意一个组都不能再被分解为更小的组时，就认为得到状态最少的DFA。
*然后将每个组中的状态合并成状态最少的DFA的一个状态。
*/
