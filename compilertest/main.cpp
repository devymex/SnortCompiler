...
int main()
{
	std::ofstream opresult("result.txt"); 
	CDllString temp("\"/(ab){2,4}c/\"");
	CPcreOption pcreOption;
	pcreOption.FromPattern(temp);
	CByteArray byteArray;
	pcreOption.Precompile(byteArray);
	
	for (ulong idx = 0; idx != byteArray.Size(); ++idx)
	{
		opresult << (ulong)byteArray[idx] << "  ";
	}
