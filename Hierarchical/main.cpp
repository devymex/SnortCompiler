...
void main(int nArgs, char **cArgs)
{
	...
	// 计算原始的跳转表大小
	for (ulong i = 0; i < CDfaSet.Size(); ++i)
	{
		oriskiptblBytes += (CDfaSet[i].Size() * CDfaSet[i].GetGroupCount());
	}
	orisumBytes = oriskiptblBytes + oricoreBytes;
	...
	std::cout << "Core matrix: " << coreBytes << std::endl;
